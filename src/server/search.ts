...
const _fetchKnowledgePanel = async (
  ...
  try {
    ...
    const res = await fetch(
      ...
      {
        headers: { "Api-User-Agent": "sneaker/1.0" },
      },
    );
...
const _makeEngineContext = (region?: string, lang?: string, dateFrom?: string, dateTo?: string): EngineContext => {
  const country = region && region !== "worldwide" ? region.toUpperCase() : undefined;
  return {
    fetch: outgoingFetch,
    region: country,
    lang: lang || undefined,
...
export const searchSingleEngine = async (
  ...
  const engineContext = _makeEngineContext(undefined, lang, dateFrom, dateTo);
  try {
...
export const search = async (
  ...
  const engineContext = _makeEngineContext(region, lang, dateFrom, dateTo);

  const settled: PromiseSettledResult<SearchResult[]>[] = await Promise.allSettled(
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (result.status === "fulfilled") {
      allResults.push(result.value as SearchResult[]);
      engineTimings.push({
        name: activeEngines[i].name,
        time: elapsed,
        resultCount: (result.value as SearchResult[]).length,
      });
    } else {
      engineTimings.push({
        name: activeEngines[i].name,
