...
const _cleanThumbnail = (thumb: string): string | undefined => {
  if (!thumb || thumb === PLACEHOLDER_GIF) return undefined;
  return thumb;
};
...
export class BingVideosEngine implements SearchEngine {
  ...
  async executeSearch(
    ...
    else if (region) url += `&setlang=${region.toLowerCase()}`;
      ...
      try {
        ...
        if (data.murl || data.vurl) {
          results.push({
            ...
            snippet: data.desc || "",
            source: this.name,
            thumbnail: _cleanThumbnail(data.turl || data.imgurl || ""),
            duration: data.dur || "",
          });
        }
      } catch {}
      ...
      $(".mc_vtvc_meta, .vrhdata").each((_, el) => {
        ...
        if (title && link) {
          results.push({
            ...
            snippet: "",
            source: this.name,
            thumbnail: _cleanThumbnail(thumb),
          });
        }
      });
    }
